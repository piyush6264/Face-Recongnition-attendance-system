...
def take_images():
    ...
    if not user_id or not user_name:
        mess.showerror(
            "Invalid Input", "Please select an ID and ensure name is filled."
        )
        return
    ...
    while count < 10:
        ...
        for x, y, w, h in faces:
            count += 1
            cv2.imwrite(
                f"TrainingImage/{user_name}.{user_id}.{count}.jpg",
                gray[y : y + h, x : x + w],
            )
    ...
    if count > 0:
        ...
        mess.showinfo(
            "Success", f"{count} Images saved for ID: {user_id}, Name: {user_name}"
        )
    else:
...
def mark_exit(user_id, user_name):
    ...
    if duration < 60:
        mess.showwarning(
            "Exit Denied", "You cannot exit less than one minute after entry."
        )
        return False
...
def track_images():
    ...
    try:
        ...
        mess.showerror(
            "Error", "ID mapping file not found. Please train the model again."
        )
        return
    ...
    while True:
        ...
        cv2.putText(
            img, timer_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
        )
        ...
        if recognition_start_time is not None:
            ...
            if recognition_time_left > 0:
                cv2.putText(
                    img,
                    f"Marking attendance in: {int(recognition_time_left)}s",
                    (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1,
                    (0, 255, 0),
...
id_combo = ttk.Combobox(
    frame_input, width=25, font=("Arial", 12), values=list(student_data.keys())
)
...
instruction = ctk.CTkLabel(
    frame_input,
    text="ℹ️ Select ID to auto-fill name",
...
log_label = ctk.CTkLabel(
    frame_log, text="📑 Attendance Log", font=ctk.CTkFont(size=18, weight="bold")
)
log_label.pack(pady=10)

attendance_list = tk.Text(
    frame_log,
    height=12,
    width=70,
    font=("Consolas", 12),
